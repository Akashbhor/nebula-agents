...
Select the product's declared backend stack once per session, then load only the references needed by the active role.
...
3. Resolve the declared backend roots and stack pack
...
- Treat non-default backend roots as first-class when they are declared in the product blueprint.
- Do not assume `engine/` unless the product explicitly uses that name.
...
- Confirm the active stack pack matches the product blueprint
