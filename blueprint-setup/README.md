...
7) Decide your implementation-layer folder names up front. The framework supports the original `engine/`, `experience/`, and `neuron/` names, but a product may use other names such as `portal/`, `api/`, or `services/` as long as `{PRODUCT_ROOT}/planning-mds/BLUEPRINT.md` and `code-index.yaml` declare them. Use the `nebula-adapter` role when the framework routing or docs need to be updated for that layout, including Vue 3 + Vite + TypeScript portal frontends.
